...
function applyTextEdit(oldWords, newText) {
  ...
  matches.forEach(function (match_, matchIdx) {
        ...
        if (matchIdx > 0) {
          ...
          var oldWordsBetween = oldWords.slice(prevOldIdx + 1 | 0, oldIdx);
          ...
          if (newCountBetween > 0) {
            if (oldCountBetween > 0) {
              ...
              if (newCountBetween > oldCountBetween) {
                ...
                var lastUsedOld = oldWordsBetween[assignCount - 1 | 0];
                var prevEnd = Core__Option.getOr(Caml_option.nullable_to_opt(lastUsedOld.timestamp[1]), lastUsedOld.timestamp[0]);
                var nextStart = oldWords[oldIdx].timestamp[0];
                var timestamps = interpolateTimestamps(prevEnd, nextStart, remainingNewTokens.length);
                remainingNewTokens.forEach(function (token, i) {
                      result.push({
                            text: token,
              ...
              var prevEnd$1 = Core__Option.getOr(Caml_option.nullable_to_opt(oldWords[prevOldIdx].timestamp[1]), oldWords[prevOldIdx].timestamp[0]);
              var nextStart$1 = oldWords[oldIdx].timestamp[0];
              var timestamps$1 = interpolateTimestamps(prevEnd$1, nextStart$1, newCountBetween);
              newTokensBetween.forEach(function (token, i) {
                    result.push({
                          text: token,
        ...
        var finalText = matchIdx === 0 && startMergeText !== "" ? startMergeText + " " + matchedToken : matchedToken;
        result.push({
              text: finalText,
